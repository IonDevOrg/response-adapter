...
function collectRequestData(request) {
    return new Promise((resolve, reject) => {
        ...
        }else if(request.headers['content-type'] && request.headers['content-type'].toLowerCase().includes('json')){
...
class WebRequestAdapter {
    ...
    constructor(request) {
        ...
        // Check if AWS:
        this.isAWSRequest = request.resource && request.path && request.httpMethod
        // Check if Normal Node:
        this.isPlainRequest = request.url && request.method && request._readableState
